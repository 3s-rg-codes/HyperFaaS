...
func TestEndToEnd(t *testing.T) {
	...
	switch requestedTest { //TODO: fallthrough?
	case 0:
...
func ForceKillAndRemoveContainer(t *testing.T) {
	err := runtime.GetCLI().ContainerKill(testCtx, testID.Id, "SIGKILL")
	...
	if err != nil {
		err = runtime.GetCLI().ContainerRemove(testCtx, testID.Id, container.RemoveOptions{Force: true})
		if err != nil {
		...
		err = runtime.GetCLI().ContainerRemove(testCtx, testID.Id, container.RemoveOptions{})
		t.Logf("Container with ID %v killed and removed", testID.Id)
	}
}

func TestRegularExecution(t *testing.T) { //TODO
	t.Logf("Starting regular execution test for container with ID %v", testID.Id)
